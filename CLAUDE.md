...
- `server.js` — single-file Express app. Responsibilities:
  ...
  - Proxies HA REST calls the browser shouldn't make directly with the token: `/api/weather`, `/api/sensor-states`, `/api/sun-moon`, `/api/camera-snapshot`.
  - `GET /api/immich/album` — proxies the Immich album asset list server-side (uses the API key), filtering to `type === 'IMAGE'`.
- `public/app.js` — all frontend logic in one file, organized by feature (search for the `// ====` section headers): clock, weather, slideshow, camera overlay, special views (dashboard/FlightAware), music overlay, and an `HAWebSocket` class.
  ...
  - **Special views**: every `specialViews.intervalPhotos` photos, the slideshow is interrupted to show one item from a rotating queue (`['flightaware', 'dashboard']`) instead of the next photo. The dashboard view shows temperature sensor cards (from `specialViews.sensorEntities`) plus sunrise/sunset/moon phase; FlightAware loads an external URL in an iframe. Each auto-dismisses back to the slideshow after its own configured duration.
